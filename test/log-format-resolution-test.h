...
TEST(LogFormatResolutionTest, NormalFormat) {
    ...
    LogFormat format(Level::Info, UNICODE_LITERAL("%logger %thread"));
    EXPECT_EQ("%logger %thread", format.userFormat());
    EXPECT_EQ("%logger %thread", format.format());
    EXPECT_EQ("", format.dateTimeFormat());

    LogFormat format2(Level::Info, UNICODE_LITERAL("%logger %datetime{%d/%M/%Y %h:%m:%s  } %thread"));
    ...
    LogFormat format3(Level::Info, UNICODE_LITERAL("%logger %datetime{%d/%M/%Y} %thread"));
...
TEST(LogFormatResolutionTest, DefaultFormat) {
    ...
    LogFormat defaultFormat(Level::Info, UNICODE_LITERAL("%logger %datetime %thread"));
    ...
    LogFormat defaultFormat2(Level::Info, UNICODE_LITERAL("%logger %datetime %thread"));
    ...
    LogFormat defaultFormat4(Level::Verbose, UNICODE_LITERAL("%logger %level-%vlevel %datetime %thread"));
...
TEST(LogFormatResolutionTest, EscapedFormat) {
    ...
    LogFormat escapeTest(Level::Info, UNICODE_LITERAL("%logger %datetime{%%H %H} %thread"));
    ...
    LogFormat escapeTest2(Level::Info, UNICODE_LITERAL("%%logger %%datetime{%%H %H %%H} %%thread %thread %%thread"));
    ...
    LogFormat escapeTest3(Level::Info, UNICODE_LITERAL("%%logger %datetime{%%H %H %%H} %%thread %thread %%thread"));
